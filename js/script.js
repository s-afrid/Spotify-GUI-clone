...
async function getSongs(folder) {
  ...
  let a = await fetch(`https://spotify-gui-clone.onrender.com/${currentFolder}/`);
...
async function displayAlbums() {
  let a = await fetch(`https://spotify-gui-clone.onrender.com/songs/`);
  let response = await a.text();
  // Create a div element and add the reponse html code inside it
  let div = document.createElement("div");
  div.innerHTML = response;
  ...
  for (let index = 0; index < array1.length; index++) {
    ...
    if (e.href.includes("/songs")) {
      ...
      if (folder_name != "songs") {
        ...
        let a = await fetch(
          `https://spotify-gui-clone.onrender.com/songs/${folder_name}/info.json`
        );
